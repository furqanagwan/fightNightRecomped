...
| Change | Repository |
| --- | --- |
| A Fight Night game: codegen config, seeds, game-only kernel stubs, settings, README, research notes | This repository, in the game's folder |
...
long (boot to menus, a full fight, career mode...). Record anything new you
...
5. Package and publish:
   ...
   ```
   This creates the tag `<GAME>-v<x.y.z>` (for example `fightNight4-v0.1.0`) and
...
Each game is released separately, so a Fight Night Round 4 fix doesn't need a new
Fight Night Champion release.
